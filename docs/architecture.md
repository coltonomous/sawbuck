...
Every listing that qualifies (buy/strong_buy) automatically receives the full treatment: a refinishing plan with materials, plus 3 finish concept renders showing different surface treatments (stain, paint, oil, etc.) if FAL_KEY is set. No per-listing caps.
...
| **planOptions** | Qwen3 32B + Bedrock | Furniture metadata | 1 plan + materials + 3 finish concepts + renders | Yes (concept_renders, refinishing_plans, materials) |
...
Platform Search ──scrapeOne──> ScrapedCandidate (title, price, images, location, platform)
                    ...
                    discoverKnowledge --> queue RAG sources for knowledge gaps
                       |
                    planOptions --> INSERT refinishing_plans + materials + concept_renders
                                   + render finish concept images (fal.ai img2img from listing photo)
                                   One plan per listing; concepts show different surface finishes
```
...
users ---------------+
  ...
  |   |              |---> conceptRenders (3 per listing, unique on (listing_id, concept_index) — concepts may share finishType)
  |   |              |---> comparables (eBay pricing data)
  |   |              |---> refinishingPlans ---> materials (single plan pre-generated by pipeline)
  |   |
...
GitHub Push (main) --> GitHub Actions
  |
  +-- Test job: pgvector service, `npm run db:migrate` (exercises scripts/migrate.ts on every PR), drizzle-kit push, vitest
  |
  +-- Deploy job:
        +-- rsync to EC2
...
1. `entrypoint.sh`:
   - Create data dirs
   - Run idempotent pre-push SQL (`scripts/migrate-new-tables.sql`) so `drizzle-kit push` never prompts on tables it can't auto-resolve
   - Apply numbered migrations via `npx tsx scripts/migrate.ts` (source of truth — see `drizzle/README.md` for the rollback story and historical-seeding behavior)
   - Run `drizzle-kit push --force` as a drift safety-net, then start server
2. Server: `assertRequiredEnv()` fails fast if `DATABASE_URL`, `BETTER_AUTH_SECRET`, `AWS_REGION`, `S3_BUCKET`, or `FAL_KEY` is missing (prod only). Then seed platform settings + default region, bootstrap RAG (incremental sync), start image cleanup scheduler. Admins are promoted manually via `npx tsx scripts/promote-admin.ts <email>` (no auto-promotion to avoid email-squatting races)
...
- **Cost-runaway cooldown**: per-(user, listing) cooldown on the expensive on-demand endpoints — 60s on `POST /api/listings/:id/analyze`, 30s on `POST /api/listings/:id/render`. Returns 429 with `Retry-After` when hit. Complements the 60/min global IP limit at `/api/*`.
- **Finish concepts**: 3 finish concept renders per listing (e.g., dark walnut stain, chalk white paint, danish oil) — visual options for the surface treatment, all sharing the same underlying plan
...
- **Pre-loaded plans**: listing detail shows a pre-generated refinishing plan with materials and finish concept renders — no on-demand generation needed
- **Project claim**: creating a project claims the existing plan and materials instead of regenerating them
