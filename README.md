...
- **Finish Concepts** — Generates concept images showing different surface treatments (stain, paint, oil, varnish, etc.) so you can visualize finish options before committing.
...
- **Refinishing Plans** — One detailed step-by-step plan per listing with product recommendations, time/cost estimates, and materials lists. Pre-generated by the pipeline — all data is ready before a user sees the listing.
...
- **Auto-Analyze on Import** — Imported listings automatically run vision analysis, pricing, and full concept/plan/materials/render generation in the background. Rate-limited to 5 imports per 10 minutes per user.
- **Smart Defaults** — Plan + 3 finish concept renders generated automatically for all qualified listings.
...
evaluate (Qwen VL vision + eBay pricing) -->
  +-- pass -> dismissed
  +-- maybe -> stays in feed
  +-- buy/strong_buy -> discoverKnowledge -> planOptions (plans + materials + renders)
